...
// 修正背景執行的全域變數：改用絕對 Timestamp 時間戳
let targetAlarm = null; 
let tomatoTargetTime = null; 
let alarmSound = new Audio(), isRinging = false;
...
function toggleTomato() {
    ...
    if (isRunning) { 
        ...
        if (timeLeft > 0) {
            isRunning = true; 
            btn.innerText = "Pause";
            tomatoTargetTime = Date.now() + timeLeft * 1000; 

            tmInterval = setInterval(() => { 
                const now = Date.now();
                timeLeft = Math.round((tomatoTargetTime - now) / 1000); 
...
function resetTomato() { 
    clearInterval(tmInterval); isRunning = false; timeLeft = 0; tomatoTargetTime = null;
    advancedCurrentIndex = -1;
    updateTomatoPreview(); 
...
function setAlarm() {
    ...
    const s = parseInt(document.getElementById('alarm-s').value);
    
    let now = new Date();
    let alarmTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), h, m, s);
    
    if (alarmTime <= now) {
        alarmTime.setDate(alarmTime.getDate() + 1);
    }
    
    targetAlarm = alarmTime.getTime(); 
    
    const timeStr = `${h.toString().padStart(2,'0')}:${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}`;
    document.getElementById('alarm-status').innerText = "Alarm set: " + timeStr;
    document.getElementById('set-alarm-btn').style.display = 'none';
...
setInterval(() => {
    ...
    if (mainClock) mainClock.innerText = curTime;
    
    if (targetAlarm && Date.now() >= targetAlarm && !isRinging) {
        startRinging('alarm'); 
        targetAlarm = null; 
    }
    ...
    if(tzSelect) {
        ...
        if (worldTimeEl) worldTimeEl.innerText = tzTimeStr;
        ...
        if(parts.length === 3) {
            ...
            const analogMin = document.getElementById('analog-min');
            const analogSec = document.getElementById('analog-sec');
            
            if(analogHour) analogHour.style.transform = `rotate(${hDeg}deg)`;
            if(analogMin) analogMin.style.transform = `rotate(${mDeg}deg)`;
            if(analogSec) analogSec.style.transform = `rotate(${sDeg}deg)`;
        }
    }
}, 500);
